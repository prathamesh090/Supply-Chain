...
// API Base URLs - use relative paths for Vite proxy
...
export const getRouteHistory = async () => {
  try {
    ...
    if (!response.ok) {
      console.warn('Failed to fetch route history:', response.status);
      return { orders: [] };
    }
    return await response.json();
  } catch (err) {
    console.warn('Route history fetch error:', err);
    return { orders: [] };
  }
};
...
export const getRouteDashboardSummary = async () => {
  try {
    ...
    if (!response.ok) {
      console.warn('Failed to fetch route dashboard summary:', response.status);
      return {
        total_orders: 0,
        fully_fulfilled: 0,
    ...
    console.warn('Route dashboard summary fetch error:', err);
    return {
      total_orders: 0,
      fully_fulfilled: 0,
      partially_fulfilled: 0,
...
export interface InventoryProduct {
  ...
  supplier_name: string;
  supplier_location: string;  // Added: supplier's location (e.g., Mumbai, Pune, Ahmedabad)
...
export const addInventoryItem = async (payload: {
  ...
  lead_time: number;
  supplier_name?: string;
  supplier_location?: string;  // Added: supplier location (Mumbai, Bhopal, Pune, etc.)
}) => {
